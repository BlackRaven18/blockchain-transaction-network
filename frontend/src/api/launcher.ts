...
const postInjectNodeDamageError = async (nodeId: string) => {
    ...
    return await axiosInstance.post(`http://${node.host}:${node.port}/api/v1/inject-node-damage-error`)
}
...
const postResetNodeDamageError = async (nodeId: string) => {
    ...
    return await axiosInstance.post(`http://${node.host}:${node.port}/api/v1/reset-node-damage-error`)
}
export {
    startLogger,
    startNetwork,
